...
<script>
    $(document).ready(function()
    {
        ...
        $(add_photo_button).click(function(e)
        {
            ...
            {
                i++;
                $(photo_wrapper).append('<div class="col-md-6"> <div class="form-group"> {{ Form::label("featured_image", "الصورة الرئيسية", ["class" => "form-control-label d-block"]) }} <div class="input-group"> <span class="input-group-btn"> <a id="uploadFile" data-input="thumbnail" data-preview="holder" class="btn btn-secondary"> <i class="fa fa-picture-o"></i> اضافة صورة </a> </span> <input id="thumbnail" class="form-control d-none" type="text" name="featured_image"> </div> </div> </div> <div class="col-md-2"> @if ($post->featured_image) <a href="{{ asset($post->featured_image) }}" target="_blank"> <img alt="Image placeholder" class="avatar avatar-xl  " data-toggle="tooltip" data-original-title="{{ $post->name }} Logo" src="{{ asset($post->featured_image) }}"> </a> @endif <a href="#" class="remove_photo_field">Remove</a> </div>');
            }
        });
